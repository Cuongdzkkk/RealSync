...
public class PropertyUpdateDtoValidator : AbstractValidator<PropertyUpdateDto>
{
    public PropertyUpdateDtoValidator()
    {
        ...
        RuleFor(x => x.PropertyCategoryId)
            .NotEqual(Guid.Empty).WithMessage("Danh mục bất động sản không hợp lệ.")
            .When(x => x.PropertyCategoryId.HasValue);

        RuleFor(x => x.PropertyTypeId)
            .NotEqual(Guid.Empty).WithMessage("Loại bất động sản không hợp lệ.")
